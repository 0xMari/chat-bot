...
export default function ChatbotPage() {
  ...
  const scrollBottom = () => {
    if(chatRef.current) {
      chatRef.current.scrollIntoView({behavior: 'smooth', block: 'end', inline: 'end',})
        ...
        {chatHistory.length>0 ? (
            <div className='flex-1 flex flex-col h-full w-full overflow-y-scroll p-6 mb-10' >
              {chatHistory.map((chat, index)=> (
                chat.role === 'user' ? (
                  <div key={index}  ref={chatRef} className='mb-5 p-4 bg-green-100 self-end text-right max-w-[40vw] min-w-[10vw] border border-green-400 self-end text-right shadow-solidGeneral'>
                    {chat.content}
                  </div>
                ) : (
